...
class reinvestBackTest(object):
    ...
    def __init__(self, time_series, avStrat='SMA', investment=1000.0, transaction_fee=0.002):
        ...
        # set up the trading vector, simply [-1 , 1]
    ...
    def __enterMarket(self, pos):
        ...
        self.__shares[pos] = (self.__portfolio[pos - 1]) / \
            self.__time_series[pos]
        self.__portfolio[pos] = (
            self.__shares[pos] * self.__time_series[pos]) * (1.0 - self.__transaction_fee)
    ...
    def __exitMarket(self, pos):
        ...
        self.__portfolio[pos] = self.__shares[pos - 1] * \
            self.__time_series[pos] * (1.0 - self.__transaction_fee)
        self.__shares[pos] = 0
        self.__costs[pos] = self.__costs[pos - 1] + (
            self.__shares[pos] * self.__time_series[pos]) * self.__transaction_fee
        # indicates a short position in the trading history
        self.__trades[pos] = -1
    ...
    def __log_return(self, pos):
        self.__log_returns[pos] = np.log(
            self.__time_series[pos] / self.__time_series[pos - 1])
    ...
    def SMA_crossOver(self):
        ...
        # bollinger bands:
        bollUp = self.__bollUp(
            self.__time_series, self.__long_mean, 2 * self.__window_long)
        ...
        for i in range((self.__window_long + 1),
            ...
            if self.__short_mean[i] > self.__long_mean[i]:
                # and self.__time_series[i] > self.__short_mean[i]: # and emergencyExit is False and  :
                if self.__position is False and self.__time_series[i] > bollUp[i]:
    ...
    def optimizeSMA(self, window_long_min, window_long_max, long_interval, window_short_min=1,
        ...
        __axis_long = np.linspace(
            window_long_min, window_long_max, long_interval)
        __axis_short = np.linspace(
            window_short_min, window_short_max, short_interval)
        self.return_mesh = np.zeros(
            [len(__axis_short) + 1, len(__axis_long) + 1])
        __i_count = 0
        ...
        for i in range(window_long_min, window_long_max, long_interval):
            ...
            for j in range(window_short_min, window_short_max, short_interval):
                ...
                if __tmp_portfolio_old[-1] < __new_portfolio[-1]:
                    ...
                    # copy.deepcopy(self.__shares)
                    __best_shares = __new_shares
                    ...
                    filename = ('best_portfolio_' + str(i) +
                                '_' + str(j) + '.csv')
    ...
    def plotStrategy(self, strat, shortwin, longwin, trigger=1):
        ...
        if strat == 'SMA':
            ...
            plt.plot(self.__bollUp(self.__time_series, self.getRollingMean(
                longwin), 2 * longwin), linewidth=1.5)
            plt.plot(self.__bollLow(self.__time_series, self.getRollingMean(
                longwin), 2 * longwin), linewidth=1.5)

            plt.legend(['Time series', 'short window',
                       'long window', 'Bollinger Up', 'Bollinger Low'])
            ...
            plt.legend(['MACD ' + str(shortwin) + '/' +
                       str(longwin), 'Trigger ' + str(trigger)])
