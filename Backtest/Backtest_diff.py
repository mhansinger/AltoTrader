...
class BackTest_diff(object):
    ...
    def __init__(self, time_series, investment=1000.0, transaction_fee=0.0016):
        ...
        self.trades = np.zeros(len(self.time_series.index))
        ...
        self.best_data = []
        self.position = False
        self.signals = pd.DataFrame(self.zeros, index=self.time_series.index, columns=[
                                    'signal', 'positions'])
        self.portfolio = pd.DataFrame(self.zeros, index=self.time_series.index, columns=[
    ...
    def enterMarket(self, pos):
        ...
        self.portfolio[pos] = (
            self.shares[pos] * self.time_series[pos]) * (1.0 - self.transaction_fee)
    ...
    def exitMarket(self, pos):
        ...
        self.portfolio[pos] = self.shares[pos - 1] * \
            self.time_series[pos] * (1.0 - self.transaction_fee)
        self.shares[pos] = 0
        self.costs[pos] = self.costs[pos - 1] + (self.shares[pos] * self.time_series[
            pos]) * self.transaction_fee
    ...
    def log_return(self, pos):
        self.log_returns[pos] = np.log(
            self.time_series[pos] / self.time_series[pos - 1])
    ...
    def SMA_crossOver(self):
        ...
        self.signals['signal'][self.window_short:] = \
            np.where(self.short_mean[self.window_short:] >
                     self.long_mean[self.window_short:], 1.0, 0.0)
        # Generate trading orders
        self.signals['positions'] = self.signals['signal'].diff()

        # bollinger bands:
        bollUp = self.bollUp(
            self.time_series, self.long_mean, 3 * self.window_long)
        ...
        self.portfolio['holdings'] = (self.shares.multiply(
            self.time_series, axis=0)).sum(axis=1)

        # Add `cash` to portfolio
        self.portfolio['cash'] = self.investment - \
            (pos_diff.multiply(self.time_series, axis=0)).sum(axis=1).cumsum()
    ...
    def plotStrategy(self, shortwin, longwin, trigger=1):
        ...
        plt.plot(self.bollUp(self.time_series, self.getRollingMean(
            longwin), 2 * longwin), linewidth=1.5)
        plt.plot(self.bollLow(self.time_series, self.getRollingMean(
            longwin), 2 * longwin), linewidth=1.5)

        plt.legend(['Time series', 'short window', 'long window',
                   'Bollinger Up', 'Bollinger Low'])
